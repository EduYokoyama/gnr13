...
class LinhaInstrumentoGeral(LinhaInstrumentoGeralTemplate):
  ...
  def btn_substituir_click(self, **event_args):
    """Substitui o instrumento atual"""
    from GNR13.GerenciarInstrumentos.LinhaInstrumento import LinhaInstrumento
    temp_linha = LinhaInstrumento()
    temp_linha.item = self.item
    
    class DummyParent:
      class DummyParent2:
        class DummyParent3:
          def atualizar_lista(inner_self):
            self.parent.parent.parent.atualizar_lista()
        parent = DummyParent3()
      parent = DummyParent2()
    temp_linha.parent = DummyParent()
    temp_linha.btn_substituir_click()
