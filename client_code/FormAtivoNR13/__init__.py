...
class FormAtivoNR13(FormAtivoNR13Template):
  def __init__(self, **properties):
    ...
    try:
      ...
      # CORREÇÃO: Incluindo o Grupo D na lista de itens permitidos
      self.drp_grupo_fluido.items = ["Grupo A", "Grupo B", "Grupo C", "Grupo D"]
    except Exception as e:
      print(f"Erro ao inicializar formulário: {e}")
  ...
  def _atualizar_dados_fluido(self, nome_fluido, contexto):
    if nome_fluido:
      ...
      if detalhes:
        if contexto == "vaso":
          # Agora o dropdown aceitará 'Grupo D' sem erro
  ...
  def btn_salvar_click(self, **event_args):
    ...
    if dados_mestre['tag'] and dados_mestre['unidade']:
      try:
        ...
        Notification("Equipamento salvo com sucesso!", style="success").show()
        self.limpar_tela()
      except Exception as e:
